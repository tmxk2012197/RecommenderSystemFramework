		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			for (Text value : values) {
				if (value.toString().contains("=")) {
					...
					relationMap.put(movieAndRelation[0], Double.parseDouble(movieAndRelation[1]));
				} else {
					String[] userAndRating = value.toString().split(":");
					ratingMap.put(userAndRating[0], Double.parseDouble(userAndRating[1]));
				}
			}
